...
def viewEntries(request):
    ...
    # list of all (distinct) project available for this user
    ...
    # list of all time entry texts (we don't want these to be distinct because we're collecting these items to then calculate the frequency of words used)
...
def updateDiagrams(request):
    if request.POST['timespan'] == 'allTime':
        return HttpResponse("All time works fine")
    elif request.POST['timespan'] == 'thisMonth':
